...
class OnboardingMonumentAuthorityUpdateModel extends AbstractTemplateModel
{
    public const TEMPLATE_KEY = 'onboarding_monument_authority_update';

    protected $entityClass = MonumentAuthority::class;

    protected $templateLabel = 'app.email_template.entity.template_key_choices.monument_authority_update';
    ...
    public function getRecipient(): ?Address
    {
        ...
        if (null === $this->recipient && $entity instanceof MonumentAuthority) {
            $type = $entity->getIntermediaryOperatorType();
            if ($type === MonumentAuthority::INTERMEDIARY_OPERATOR_TYPE_2) {
...
Intermediär = „DataClearing NRW - Zuordnung nicht bekannt“ => Empfänger (Standard)');
        $template->setSubject('Bauportal.NRW - XTA-Antrag  ###ONBOARDING_MONUMENT_AUTHORITY_COMMUNE### - Daten via KDN-Formular');
        $template->setBody('Die XTA Onboarding Information von  ###ONBOARDING_MONUMENT_AUTHORITY_COMMUNE### wurden von ###USER_FIRSTNAME### ###USER_LASTNAME### aktualisiert:
###ONBOARDING_MONUMENT_AUTHORITY_ADMIN_EDIT_URL###
###CHANGES###');
        return $template;
    }
    ...
    {
        $changesContent = '';
        if ($object instanceof MonumentAuthority) {
